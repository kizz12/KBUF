...
function login() --can be called to require a user to log in.

	while access~=1 do
	print'Enter Password:'
	pass=read("*")
	access=0
	staticPass="nou" --Set pass here
		if pass~=staticPass and pass~="guest" then 
		print'Wrong Password!'
		end
		sleep(1)
		term.clear()
		term.setCursorPos(1,1)
		if pass==staticPass then
		access=1
	...
	function logme() --logs user activity
